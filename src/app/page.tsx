...
export default async function Home() {
  ...
  const settings =
    resolvedTenant.settings as ExtendedTenantSettings | null;
  ...
  const [
    ...
    db
      .from("tenant_contact_form_settings")
      .select(
        `
          title,
          description,
  ...
  const gallery =
    (galleryResult.data ?? []) as PublicGalleryItem[];

  const beforeAfter =
    (beforeAfterResult.data ?? []) as BeforeAfterItem[];

  const reviews =
    (reviewsResult.data ?? []) as Review[];

  const faq =
    (faqResult.data ?? []) as FaqItem[];

  const form =
    formResult.data as ContactFormSettings | null;

  /*
   * Analytics:
   * se a tabela existir, registra a visualização.
   * Não impede a renderização da landing caso o insert falhe.
   */
  ...
  const accent =
    settings?.accent_color ?? "#f59e42";

  const primary =
    settings?.primary_color ?? "#0c1d32";
  ...
  return (
    <main
      style={cssVariables}
      className="min-h-screen bg-[var(--surface)] text-slate-900"
    >
      {/* NAVBAR */}
      ...
      <section
        id="topo"
        className="hero-grid scroll-mt-20 border-b border-slate-200 bg-slate-50"
      >
      ...
      {services.length > 0 && (
        <section
          id="servicos"
          className="scroll-mt-20 bg-slate-100 py-20 sm:py-24"
        >
              ...
              {faq.map((item) => (
                ...
                >
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-6 font-semibold text-slate-900">
                    ...
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xl font-normal transition group-open:rotate-45">
                      +
                    </span>
                  </summary>
        ...
        >
          <div className="mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
            ...
            >
              ...
              <button
                type="submit"
                className="mt-1 inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-6 py-3.5 font-semibold text-white transition hover:-translate-y-0.5 hover:opacity-90"
              >
                <FaWhatsapp />
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-6 py-8 text-center text-sm text-slate-500 sm:flex-row sm:text-left">
          ...
          <span>
            © {new Date().getFullYear()} {company}. Todos
